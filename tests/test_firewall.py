...
def test_firewall(project, vy_host, clear):
    ...
    # address group
    a_group = project.get_resource("vyos::Config", node = "firewall group address-group test-a-group")
    assert a_group

    a_group_cfg = a_group.config.splitlines()
    assert len(a_group_cfg) == 3
    assert "firewall group address-group test-a-group address 192.168.25.6" in a_group_cfg
    assert "firewall group address-group test-a-group address 192.168.26.8-192.168.26.10" in a_group_cfg
    assert "firewall group address-group test-a-group description \"inmanta managed address-group\"" in a_group_cfg

    a_group_deploy = project.deploy(a_group)
    assert a_group_deploy.status == inmanta.const.ResourceState.deployed

    # network group
    n_group = project.get_resource("vyos::Config", node = "firewall group network-group test-n-group")
    assert n_group

    n_group_cfg = n_group.config.splitlines()
    assert len(n_group_cfg) == 2
    assert "firewall group network-group test-n-group network 192.168.27.0/24" in n_group_cfg
    assert "firewall group network-group test-n-group description \"inmanta managed network-group\"" in n_group_cfg

    n_group_deploy = project.deploy(n_group)
    assert n_group_deploy.status == inmanta.const.ResourceState.deployed

    # port group
    p_group = project.get_resource("vyos::Config", node = "firewall group port-group test-p-group")
    assert p_group
    ...
    assert "firewall group port-group test-p-group description \"inmanta managed port-group\"" in p_group_cfg
    ...
    assert "firewall name test-set-1 description \"inmanta managed ruleset\"" in ruleset_cfg
    ...
    assert "firewall name test-set-1 rule 10 description \"inmanta managed rule\"" in ruleset_cfg
    ...
    assert "firewall name test-set-1 rule 20 description \"inmanta managed rule\"" in ruleset_cfg
    assert "firewall name test-set-1 rule 20 source group address-group test-a-group" in ruleset_cfg
    assert "firewall name test-set-1 rule 20 source group port-group test-p-group" in ruleset_cfg
    assert "firewall name test-set-1 rule 20 destination group network-group test-n-group" in ruleset_cfg
    assert "firewall name test-set-1 rule 20 destination group port-group test-p-group" in ruleset_cfg
